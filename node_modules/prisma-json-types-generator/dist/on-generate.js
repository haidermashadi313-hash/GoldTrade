...
async function onGenerate(options) {
    try {
        ...
        const clients = (0, prisma_generator_1.findPrismaClientGenerators)(options.otherGenerators);
...
async function generateClient(prismaClient, config, options) {
    ...
    if (isNewClient) {
        ...
        if (stat?.isDirectory()) {
            for (const modelFile of await promises_1.default.readdir(modelsFolder)) {
                await handleDeclarationFile((0, node_path_1.join)(modelsFolder, modelFile), config, options, ext, true);
            }
            await promises_1.default.writeFile((0, node_path_1.join)(prismaClient.output.value, 'pjtg.ts'), await (0, declaration_writer_1.getNamespacePrelude)({
